...
const updatePlaceNameById = async (id, newPlaceName) => {
  if (isNaN(id)) {
    const err = new ErrorDetails("ApartmentIconicPlaceError", "id", [
  ...
  if (!iconicPlace) {
    const err = new ErrorDetails("ApartmentIconicPlaceError", "id", "place_name not found");
    // TODO: ganti console ke log kalau sudah mau production
    console.error(err);
  ...
  try {
    ...
    // TODO: ganti console ke log kalau sudah mau production
    console.error(error);
    const err = new ErrorDetails("ApartmentIconicPlaceError", "place_name", error.message);
    throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
  }
}
